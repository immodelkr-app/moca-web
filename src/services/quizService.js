 ...
 * 따라서 정답(correct_answer) 비공개는 RLS가 아니라 "발표 전에는 클라이언트가
 * correct_answer 컬럼을 아예 select하지 않는다"는 규칙으로만 지켜진다.
 * fetchOpenQuizzes / fetchQuizDetail은 절대 correct_answer를 select하면 안 된다.
 * (알려진 한계: 서버가 아니라 클라이언트 쿼리 수준의 방어이며, 완전한 보안은 아님)
 */
import { supabase } from './supabaseClient';

const SAFE_QUIZ_COLUMNS =
    'id, question, question_type, choices, prize_description, status, winner_nicknames, created_at, closed_at, announced_at';

// ─────────────────────────────────────────────
//  사용자용
// ─────────────────────────────────────────────

/** 공개 퀴즈 목록 (correct_answer 절대 미포함) */
...
/** 답변 제출 (중복 제출은 호출부에서 error.code === '23505'로 처리) */
export const submitAnswer = async (quizId, userNickname, answerText) => {
    if (!supabase) return { submission: null, error: new Error('Supabase 미설정') };
    const { data, error } = await supabase
        .from('quiz_submissions')
        .insert([{ quiz_id: quizId, user_nickname: userNickname, answer_text: answerText }])
        .select()
        .single();
    if (error) {
...
/**
 * 발표된(status='announced') 퀴즈만 correct_answer 포함 조회.
...
export const fetchAnnouncedQuizDetail = async (quizId) => {
    ...
    const { data, error } = await supabase
        .from('quiz_posts')
        .select(`${SAFE_QUIZ_COLUMNS}, correct_answer`)
        .eq('id', quizId)
        .eq('status', 'announced')
        .maybeSingle();
    if (error) {
...
/** 퀴즈 생성 */
export const createQuiz = async ({ question, questionType, choices, prizeDescription, correctAnswer }) => {
...
/** 전체 퀴즈 목록 (correct_answer 포함 — 관리자 전용 화면이므로 select('*') 허용) */
 ...
 * status='announced'로 전환한다. 이 시점부터 fetchAnnouncedQuizDetail로 correct_answer 조회 가능.
 */
