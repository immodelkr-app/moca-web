...
/**
 * 모델뷰티의 라이브방송(지금 진행 중) + 예고(방송 예정) 목록을 함께 가져온다.
 ...
 */
export const fetchModelBeautyLiveStreams = async () => {
    if (!supabase) return { live: [], upcoming: [] };
...
// 기존 호출부 호환용 - 지금 진행 중인 라이브만 필요할 때
export const fetchLiveStreams = async () => (await fetchModelBeautyLiveStreams()).live;
